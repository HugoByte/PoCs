    ...
    match Handle::try_current() {
        Ok(r) => {
            ...
            block_on(r.spawn_blocking(move || {
                let mut client = Client::builder().pool_idle_timeout(Some(Duration::MAX));
        ...
        Err(_) => {
            ...
            let mut client = reqwest::blocking::Client::builder().timeout(Some(Duration::from_millis(300000)));
            let headr = headers.get("Upgrade-Insecure-Requests").unwrap();
            if headr.to_str().unwrap() == "1"{
                client = client.danger_accept_invalid_certs(true);
            }
